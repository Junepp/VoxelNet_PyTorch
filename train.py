...
def run():
    ...
    # Build data loader
    dataset = Dataset(os.path.join(cfg.DATA_DIR, 'training'), shuffle=True, aug=False, is_testset=False)

    dataset_len = len(dataset)
    train_size = int(dataset_len * 0.8)
    valid_size = dataset_len - train_size
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for (i, data) in enumerate(tqdm.tqdm(train_dataloader)):
            ...
            #         f'Train: {counter} @ epoch:{epoch + 1}/{args.max_epoch} loss: {loss.item():.4f} ' \
            #         f'reg_loss: {reg_loss.item():.4f} cls_loss: {cls_loss.item():.4f}' \
            #         f' cls_pos_loss: {cls_pos_loss_rec.item():.4f} cls_neg_loss: {cls_neg_loss_rec.item():.4f}\n'
            #     # f' forward time: {forward_time:.4f} batch time: {batch_time:.4f}\n'
            #
            #     print(info)
            #
        ...
        avg_loss = sum(loss_table) / len(loss_table)
        print(f'{epoch+1} epoch, loss: {avg_loss}')

        torch.save(model.state_dict(), f'/home/ryan/workspace/VoxelNet_PyTorch/saved/trial/state_dict.pt')
        torch.save(model.module.state_dict(), f'/home/ryan/workspace/VoxelNet_PyTorch/saved/trial/state_module_dict.pt')
        torch.save(model.module.state_dict(), f'/home/ryan/workspace/VoxelNet_PyTorch/saved/trial/state_module_dict.pth.tar')
...
if __name__ == '__main__':
    ...
    os.makedirs('saved/trial', exist_ok=True)
